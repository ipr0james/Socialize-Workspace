...
public final class SubCommandPVPDisable extends SpigotCommand<User> implements CommandPermission<User> {

    public SubCommandPVPDisable() {
        super("disable");
    }
    ...
    public final void execute(final User user, final CommandContext commandContext) {
        ...
        user.getModule(UserPVP.class).update(new KeyPVPDisabled(), true);
        MessageHandler.INSTANCE.build("module.togglepvp.disabled").send(user);
    }

    @Override
    public final boolean hasPermission(final User user) {
        return user.hasPermission("titan.command.pvp.disable");
    }
}