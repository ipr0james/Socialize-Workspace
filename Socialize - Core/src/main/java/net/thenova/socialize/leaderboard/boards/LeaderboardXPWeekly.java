 ...
 */
public final class LeaderboardXPWeekly extends Leaderboard {

    public LeaderboardXPWeekly(long guildID) {
        super(guildID, GuildChannelsData.Type.LEADERBOARD_XP, "Weekly XP", "xp");
    }
    ...
    public Map<String, Long> fetch() {
        ...
        new SQLQuery(new DBSocialize(), "SELECT `entity_data`.`user_id`, `entity_stats`.`stat_value` FROM `entity_data`"
                ...
                + " WHERE (`entity_data`.`guild_id`, `entity_stats`.`stat_key`) = (?, 'xp_week')"
