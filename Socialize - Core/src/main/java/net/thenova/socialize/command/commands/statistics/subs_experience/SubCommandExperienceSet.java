...
@CommandUsage(
        min = 2,
        usage = "experience set <@user> <amount>",
        description = "Set a users experience to a given value"
)
public final class SubCommandExperienceSet extends Command {

    public SubCommandExperienceSet() {
        super("set");
    ...
    protected void execute(final Entity sender, final CommandContext context) {
        ...
        if(context.getArgument(1).contains("-")) {
            context.error("You cannot set a negative value.");
            return;
        }
        ...
        entity.getLevel().set(amount, EntityLevel.Reason.ADMIN);
        context.reply(Embed.socialize("<@" + entity.getUserID() + "> experience has been set to "
                        + amount + Bot.INSTANCE.getEmoji("xp")))
                .queue();
    }
}