...
@CommandUsage(
        min = 2,
        usage = "coins take <@user> <amount>",
        description = "Take coins from a user"
)
public final class SubCommandCoinsTake extends Command {

    public SubCommandCoinsTake() {
        super("take");
    ...
    protected void execute(final Entity sender, final CommandContext context) {
        ...
        if(entity.getBalance().fetch() - amount < 0) {
            context.error("<@" + entity.getUserID() + "> only has " + entity.getBalance().fetch() + Bot.INSTANCE.getEmoji("coins"));
            return;
        }

        entity.getBalance().take(amount, EntityBalance.Reason.ADMIN);
