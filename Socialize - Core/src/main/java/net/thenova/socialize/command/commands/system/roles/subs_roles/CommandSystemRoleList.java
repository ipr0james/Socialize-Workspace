...
@CommandUsage(
        min = 0,
        usage = "system role list",
        description = "List the available channel types."
)
public final class CommandSystemRoleList extends Command {

    public CommandSystemRoleList() {
        super("list");
    }

    @Override
    protected final void execute(final Entity entity, final CommandContext context) {
        context.reply(Embed.socialize().appendDescription("Valid types: " + Arrays.stream(GuildRoleData.Type.values())
                .map(Enum::name)
