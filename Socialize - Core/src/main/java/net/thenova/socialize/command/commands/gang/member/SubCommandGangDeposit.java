...
@CommandUsage(
        min = 1,
        usage = "gang deposit <amount>",
        description = "Deposit money in to the Gang Bank."
)
public final class SubCommandGangDeposit extends CommandTemplateGang {

    public SubCommandGangDeposit() {
        super("deposit", GangRole.MEMBER);
    }

    @Override
    public final void run(final Gang gang, final Entity entity, final CommandContext context) {
        gang.getBank().deposit(context, entity);
    }
}
