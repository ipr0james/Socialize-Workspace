...
@CommandUsage(
        min = 2,
        usage = "coins set <@user> <amount>",
        description = "Set a users coins to a given value"
)
public final class SubCommandCoinsSet extends Command {

    public SubCommandCoinsSet() {
        super("set");
    ...
    protected void execute(final Entity sender, final CommandContext context) {
        ...
        entity.getBalance().set(amount, EntityBalance.Reason.ADMIN);
        context.reply(Embed.socialize("<@" + entity.getUserID() + "> balance has been set to "
                        + amount + Bot.INSTANCE.getEmoji("coins")))
                .queue();
    }
}