...
@CommandUsage(
        min = 1,
        usage = "gang demote <@member>",
        description = "Demote a member of the gang from Officer to Member"
)
public final class SubCommandGangDemote extends CommandTemplateGang {

    public SubCommandGangDemote() {
        super("demote", GangRole.LEADER);
    }
    ...
    public final void run(final Gang gang, final Entity entity, final CommandContext context) {
        ...
        final Entity demoted = EntityHandler.INSTANCE.getEntity(mentioned.get(0));
        if(demoted == null) {
            GangManager.INSTANCE.log(gang, "[SubCommandGangDemote] - Mentioned member was not a part of the guild.");
            return;
        }

        gang.getMembers().demote(context, entity, demoted);
    }
}