...
@CommandUsage(
        min = 0,
        usage = "system channel listtype",
        description = "List the available channel types."
)
public final class CommandSystemChannelListType extends Command {

    public CommandSystemChannelListType() {
        super("listtype", "list");
    }

    @Override
    protected final void execute(final Entity entity, final CommandContext context) {
        context.reply(Embed.socialize().appendDescription("Valid types: " + Arrays.stream(GuildChannelsData.Type.values())
                .map(Enum::name)
