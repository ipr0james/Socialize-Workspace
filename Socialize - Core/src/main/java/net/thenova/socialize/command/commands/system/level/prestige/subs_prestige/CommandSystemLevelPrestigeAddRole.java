...
@CommandUsage(
        min = 2,
        usage = "system level prestige addrole {prestige} {role_id}",
        description = "Add a role to a given prestige"
)
public final class CommandSystemLevelPrestigeAddRole extends Command {

    public CommandSystemLevelPrestigeAddRole() {
        super("addrole", "add");
    }
    ...
    protected final void execute(final Entity entity, final CommandContext context) {
        ...
        for(int i = 0; i < array.length(); i++) {
            if(array.large(i).equals(roleID)) {
                context.error("The role <@" + roleID + "> is already a part of Prestige " + levelNum);
                return;
            }
        }
        ...
        context.reply(Embed.socialize("Role <@" + roleID + "> has been added to Prestige " + levelNum + ", all existing users will be updated."));
    }
}
